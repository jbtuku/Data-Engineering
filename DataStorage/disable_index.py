...
def create_table(conn):
    with conn.cursor() as cur:
        ...
        """)
        print(f" Created table {TableName} (no constraints or indexes yet)")
...
def recreate_constraints_and_indexes(conn):
    with conn.cursor() as cur:
        cur.execute(f"ALTER TABLE {TableName} ADD PRIMARY KEY (TractId);")
        cur.execute(f"CREATE INDEX idx_{TableName}_State ON {TableName}(State);")
    print(" Primary Key and Index created after data load.")
...
def main():
    ...
    recreate_constraints_and_indexes(conn)
    conn.close()
